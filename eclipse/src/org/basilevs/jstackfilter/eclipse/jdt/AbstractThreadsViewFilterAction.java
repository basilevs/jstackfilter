 ...
 *     Vasili Gulevich - copied and adapted to Jstackfilter
 *******************************************************************************/
package org.basilevs.jstackfilter.eclipse.jdt;
 ...
 */
public abstract class AbstractThreadsViewFilterAction extends ViewFilterAction implements IDebugEventSetListener {
		...
		if (thread != null) {
			try {
				if (isCandidateThread(thread)) {
					...
					getView().getViewSite().getShell().getDisplay().asyncExec(r);
					return;
				}
			}
			catch (DebugException e) {
